...
CubeHeuristics.manhattanDistanceOfCorners = function(rubixState)
{
    ...
    for (var i = 0; i < RubixState.cubieMap.length; i++)
    {
        // Only check corners
        if (RubixState.cubieMap[i].length !== 3)
            continue;
            
        moves = 0;
        goalIndex = RubixState.findCubie(CubeHeuristics.goalState, i);
        
        // If it's a far corner, need at least 2 moves to correct it's position.
        if (CubeHeuristics.farCorners[i] === goalIndex)
            moves = 2;
        // Otherwise, if it's not already in the correct position.
        else if (i !== goalIndex)
            moves = 1;
        ...
        {
            // TODO fix condition!
            if (!rubixState.cubies[i].equals(CubeHeuristics.goalState.cubies[i]))
                moves += 2;
        }
        else if (moves === 1)
        {
